...
class LyLoadingMoreSliverList<T> extends StatelessWidget {
  final LySliverListConfig<T> sliverListConfig;

  LyLoadingMoreSliverList(this.sliverListConfig, {Key key}) : super(key: key);
...
class _LoadingMoreCustomScrollViewState
  ...
  List<LyLoadingMoreSliverList> _loadingMoreWidgets;
  @override
  void initState() {
    _loadingMoreWidgets = widget.slivers
        .where((x) {
          return x is LyLoadingMoreSliverList;
        })
        .map<LyLoadingMoreSliverList>((f) => f as LyLoadingMoreSliverList)
        .toList();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (loadingMoreWidgets.length > 0) {
      ...
      for (int i = 0; i < slivers.length; i++) {
        var item = slivers[i];
        widgets.add(item);
        if (item is LyLoadingMoreSliverList) {
...
//            showFullScreenLoading = false;
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (notification.metrics.pixels >= notification.metrics.maxScrollExtent) {
      ...
      if (loadingMoreWidgets.length > 0) {
        LyLoadingMoreSliverList preList;
